...
void DW1000RangingClass::startAsAnchor(char address[], const byte mode[]) {
	...
	_currentShortAddress[0] = _currentAddress[6];
	_currentShortAddress[1] = _currentAddress[7];
...
void DW1000RangingClass::checkForInactiveDevices() {
	for(uint8_t i = 0; i < _networkDevicesNumber; i++) {
		if(_networkDevices[i].isInactive()) {
			...
			// Serial.print("NUM_DEVICES in act "); Serial.println(_networkDevicesNumber);
	...
	if(_sentAck) {
		...
		if(_type == ANCHOR) {
			...
			if(messageType == POLL) {
				...
				// Serial.print(_currentShortAddress[0]); 
				// Serial.print(" "); 
				// Serial.println(_currentShortAddress[1]); 
	...
	if(_receivedAck) {
		...
		if(messageType == BLINK && _type == ANCHOR) {
			...
			if(addNetworkDevices(&myTag)) {
				// Serial.print("NUM_DEVICES "); Serial.println(_networkDevicesNumber);
			...
			if(_type == ANCHOR) {
				...
				if(messageType != _expectedMsgId) {
					...
					//not needed ?
					// Serial.println("RCVD unexpected");
					return;
					_expectedMsgId = POLL_ACK;
					return;
				}
				if(messageType == POLL_ACK) {
					// Serial.println("RCVD  POLL_ACK");
					...
					// Serial.println("RCVD RANGE_REPORT");
					float curRange;
					...
					// Serial.println("RCVD RANGE FAILED");
